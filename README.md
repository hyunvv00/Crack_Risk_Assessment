...
```text
Crack_Detection/
├── crack_val_risk.py              # 5단계 위험도 분석 (폭 기준) [file:102]
├── crack_val_angel.py             # 균열 각도 시각화 [file:107]
├── crack_val_analysis.py          # 기본 균열 분석 + 보간 [file:104]
├── crack_val_multi_analysis.py    # 고급 형태/위험도 복합 분석 ★ [file:103]
├── crack_val_prediction.py        # 예측 결과 시각화 [file:105]
├── crack_val_video.py             # 비디오 실시간 처리 [file:106]
└── keompyuteo_bijeon.pptx.pdf     # 상세 기술 문서 [file:108]
```

**권장 실행 순서**: `multi_analysis.py` → `risk.py` → `video.py`
...
### 1. 환경 설정
```bash
pip install ultralytics opencv-python scikit-image scikit-learn numpy
```

### 2. 모델 준비
```
runs/segment/train/weights/best.pt  # YOLO11 세그멘테이션 모델
# 또는 Ultralytics 허브에서 다운로드
```

### 3. 이미지 분석 실행
```bash
# 고급 분석 (권장)
python crack_val_multi_analysis.py
...
**출력**: `./crackdetectionoutput*/` 폴더에 어노테이션 이미지 저장[12][11]
